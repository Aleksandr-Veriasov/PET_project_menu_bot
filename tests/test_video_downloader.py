...
def test_download_video_and_description_success(mocker):
    ...
    mock_yt_dlp = mocker.patch('app.video_downloader.yt_dlp.YoutubeDL')
...
def test_get_video_resolution_success(mocker):
    ...
    mock_probe = mocker.patch('app.video_downloader.ffmpeg.probe')
...
def test_convert_to_mp4_success(mocker):
    ...
    mock_ffmpeg = mocker.patch('app.video_downloader.ffmpeg.input')
    mock_ffmpeg.return_value.output.return_value.run.return_value = None

    mock_get_resolution = mocker.patch(
        'app.video_downloader.get_video_resolution'
    )
...
async def test_send_video_to_channel_success(mocker):
    ...
    mock_bot = mocker.patch(
        'app.video_downloader.CallbackContext.bot',
    ...
    mocker.patch('app.video_downloader.os.getenv', return_value='test_chat_id')
...
async def test_send_video_to_channel_error(mocker):
    ...
    mock_bot = mocker.patch('app.video_downloader.CallbackContext.bot')
