...
import app.api.deepseek_api
from app.api.deepseek_api import (
    extract_recipe_data_with_deepseek,
...
def test_extract_recipe_data_with_deepseek_success(mocker):
    mock_client = mocker.patch(
        'app.api.deepseek_api.client.chat.completions.create'
    )
...
def test_api_key_loaded(mocker):
    ...
    importlib.reload(app.api.deepseek_api)  # Перезагрузили модуль

    # Теперь замокали OpenAI после reload
    mock_openai = mocker.patch('app.api.deepseek_api.OpenAI')

    # Создаём клиента заново после мока
    from app.api.deepseek_api import OpenAI
